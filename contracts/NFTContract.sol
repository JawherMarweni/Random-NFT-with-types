...
contract newerRandom {
    struct Interval {
        uint256 inclusiveLowerBorder;
        uint256 inclusiveUpperBorder;
        uint256 nextIndex;
    }

    mapping(uint256 => Interval) public intervals;
    mapping(uint256 => mapping(uint256 => uint256)) public recorded;

    constructor(
        uint256[] memory inclusiveLowerBorders,
        uint256[] memory inclusiveUpperBorders
    ) {
        require(
            inclusiveLowerBorders.length == 3 && inclusiveUpperBorders.length == 3,
            "Invalid number of intervals"
        );

        for (uint256 i = 0; i < 3; i++) {
            require(
                inclusiveLowerBorders[i] <= inclusiveUpperBorders[i],
                "Invalid interval"
            );

            intervals[i].inclusiveLowerBorder = inclusiveLowerBorders[i];
            intervals[i].inclusiveUpperBorder = inclusiveUpperBorders[i];
            intervals[i].nextIndex = inclusiveLowerBorders[i];
        }
    }

    function getNextRandom(uint256 intervalIndex)
        internal
        returns (uint256 _randomNumber)
    {
        ...
        require(
            ...
            "Used every index in the interval"
        );

        if (interval.nextIndex < interval.inclusiveUpperBorder) {
            uint256 rand =
                (uint256(
                    keccak256(
                        abi.encodePacked(
                    ...
                    (interval.inclusiveUpperBorder - interval.nextIndex + 1)) +
                interval.nextIndex;
            _randomNumber =
                recorded[intervalIndex][rand] > 0
                    ? recorded[intervalIndex][rand]
                    : rand;
            recorded[intervalIndex][rand] =
                recorded[intervalIndex][interval.nextIndex] > 0
                    ? recorded[intervalIndex][interval.nextIndex]
                    : interval.nextIndex;
            interval.nextIndex++;
        } else {
            _randomNumber =
...
contract NFTContract is newerRandom,ERC721 {
    ...
    uint256 public type2TotalSupply;
